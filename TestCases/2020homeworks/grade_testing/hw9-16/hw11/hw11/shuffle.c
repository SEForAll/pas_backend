...
// program.
void helper(CardDeck leftDeck, CardDeck rightDeck,int left,int right,CardDeck *result){
  if(left == leftDeck.size){
    //printf("Use up left deck\n");
  ...
  helper(leftDeck,rightDeck,left+1,right,result);
  result->cards[left+right] = rightDeck.cards[right];
  helper(leftDeck,rightDeck,left,right+1,result);
  return;
}
...
{  
  ...
  helper(leftDeck,rightDeck,0,0,result);
  free(result);
}
#endif
